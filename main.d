...
ulong fetch_file_content(ulong ppn,char[]sn,uint wrdcnt,UFDent[]slot,string pathname){
  ...
  try {
    blob = read("large/data8/sn/"~sn);
  }
  catch (Throwable){
    stderr.writefln("read file FAILED "~"large/data8/sn/"~sn);
    return 0;
  }
...

// path /data/Ralph_filebase/main.d
// Max ------------------------------------------------------------------------------------------- line length
int main()
{
  stdout.writefln("\n=== BEGIN ===");
...

  ...
  */
  ...
  stdout.writeln("\n===  Writing /large/DISK.data8 ===");
  auto outfile = File("large/DISK.data8","wb");
