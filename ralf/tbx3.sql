...
SELECT unixname,max(v)version FROM mz
        WHERE left(dt,4)>=1970
         and  left(dt,4)<= @yearmax
         and  unixname rlike "^SYS/(CSP|J17|K17)/"
         and ext!="old" and ext!="msg" and ext!="pur"
         group by unixname;
...
delete from target where ext='xgp' or ext='old';
  drop table if exists sys3;
rename table target to sys3;

# 45 versions of E.DMP
insert sys3 (dt,prj,prg,filnam,ext,sn,wrdcnt,tbx)
...
tbx,
lpad(sn,6,'0'),
dt from sys3 order by prg,prj,filnam,ext
