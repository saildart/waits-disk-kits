# Build file system for the SAIL WAITS public files logical device SYS:
# --------------------------------------------------------------
...
SELECT unixname,max(v)version FROM mz
        WHERE left(dt,4)>=1970
         and  left(dt,4)<= @yearmax
         and  unixname rlike "^SYS/(CSP|F40|J17|LSP)/"
         and ext!="old" and ext!="msg" and ext!="pur"
         group by unixname;
...
into OUTFILE '/var/tmp/SYS.csv' FIELDS TERMINATED BY ',' ENCLOSED BY '' LINES TERMINATED BY '\n';
# ---------------------------------------------------------------------------------------------------
# EOF.
